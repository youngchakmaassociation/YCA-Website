    ...
    return (
        <div className="container mx-auto px-4 py-8 md:py-16">
            ...
            {activeElections.length > 0 && (
                <div className="mb-32 space-y-12 animate-fade-in-up">
                    <div className="flex items-center gap-4">
                        <span className="size-3 bg-accent rounded-full animate-ping"></span>
                        <h2 className="text-2xl font-black text-primary uppercase tracking-widest">Live & Upcoming</h2>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        {activeElections.map((ele, i) => (
                            <Link key={ele._id || i} href={`/election/${ele._id || i}`} className="p-12 rounded-[3.5rem] bg-primary text-white shadow-2xl hover:scale-[1.02] transition-all group overflow-hidden relative text-left">
                                <div className="absolute top-0 right-0 size-40 bg-white/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
                                <h3 className="text-3xl font-black mb-4 pr-10 uppercase">{ele.title}</h3>
                                <div className="flex flex-wrap gap-4 mb-8">
                                    <span className="px-5 py-2 bg-white/10 rounded-full text-[10px] font-black uppercase tracking-widest">{ele.type || 'Election'}</span>
                                    {ele.status && <span className="px-5 py-2 bg-accent/20 text-accent rounded-full text-[10px] font-black uppercase tracking-widest border border-accent/30">{ele.status}</span>}
                                </div>
                                <div className="flex items-center justify-between pt-8 border-t border-white/10">
                                    <div className="flex flex-col text-left">
                                        <span className="text-[10px] font-black uppercase opacity-60 tracking-widest">Election Date</span>
                                        <span className="font-bold">{formatDate(ele.startDate)}</span>
                                    </div>
                                    <div className="size-14 rounded-2xl bg-accent text-white flex items-center justify-center group-hover:rotate-45 transition-transform shadow-xl">
